...
## 2025-02-12 - [Synchronous O(N) Iteration in High-Throughput Middleware]
**Learning:** The application's `circuitBreakerMiddleware` runs on every single incoming HTTP request. Inside this middleware, `globalFileQueue.getQueueSize()` and `getQueueBytes()` were repeatedly performing synchronous O(N) loops over the entire `itemsCache` array to count pending items and sum their bytes. While intermediate array allocations were previously removed, looping through the cache thousands of times per second still blocks the Node.js event loop on the main thread and introduces CPU bottlenecks under high concurrency.
**Action:** Replaced O(N) loop execution during read paths with O(1) cached property lookups (`this.cachedQueueSize`, `this.cachedQueueBytes`). Created a private `updateMetrics()` method that only performs the O(N) calculation when the cache is explicitly refreshed or mutated (`init`, `getItems`, `saveItems`), fully eliminating the per-request event loop blocking.
