...
def _print_success(cid: str, status: str, expires: str, tx_id: Optional[str] = None) -> None:
    ...
    click.echo(_Colors.success(f"✅ Pin successful!"))
    click.echo(_Colors.info(f"   CID:     {cid}"))
    click.echo(_Colors.info(f"   Status:  {status}"))
    if expires:
        click.echo(_Colors.info(f"   Expires: {expires}"))
    if tx_id:
        click.echo(_Colors.info(f"   TX ID:   {tx_id}"))
...
def pin(ctx, file_path, filename, max_price, assume_yes):
    ...
    click.echo(_Colors.info(f"📦 Pinning {fp}..."))
...
def pin_url(ctx, url, filename, max_price, assume_yes):
    ...
    click.echo(_Colors.info(f"📥 Downloading {url}..."))

    try:
        import requests as req
        resp = req.get(url, timeout=30)
        resp.raise_for_status()
        click.echo(_Colors.info(f"  Downloaded {len(resp.content)} bytes"))
    except Exception as e:
...
def cmd_status(ctx, cid):
    ...
    click.echo(_Colors.info(f"🔍 Checking status for {cid}..."))
    ...
    try:
        ...
        if status in ("pinned", "confirmed"):
            click.echo(_Colors.success(f"✅ {status}"))
        elif status == "unpinned":
            click.echo(_Colors.warning(f"⚠️  {status}"))
        else:
            click.echo(_Colors.muted(f"• {status}"))
    except Exception as e:
        _print_error(str(e))
        sys.exit(1)
...
def renew(ctx, cid, max_price, assume_yes):
    ...
    click.echo(_Colors.info(f"🔄 Renewing pin for {cid}..."))
...
def unpin(ctx, cid, assume_yes):
    ...
    click.echo(_Colors.info(f"🗑️  Unpinning {cid}..."))
    ...
    try:
        ...
        if resp.status_code in (200, 204):
            click.echo(_Colors.success(f"✅ Unpinned {cid}"))
        else:
...
def list_pins(ctx):
    ...
    click.echo(_Colors.info("📋 Listing pins..."))
        ...
        if resp.status_code == 200:
            ...
            if isinstance(pins, list) and pins:
                ...
                for pin in pins:
                    ...
                    click.echo(f"  {_Colors.muted(cid)}")
                    click.echo(f"    status: {_Colors.success(status) if status == 'pinned' else status}")
                    if created:
                        click.echo(f"    created: {created}")
                    click.echo("")
            else:
                click.echo(_Colors.muted("No pins found."))
        elif resp.status_code == 404:
            click.echo(_Colors.muted("No pins endpoint available. Try connecting to a gateway with Supabase."))
        else:
...
def config_set(ctx, key, value):
    ...
    if key not in ("gateway", "mnemonic", "evm_private_key", "solana_private_key"):
        click.echo(_Colors.error(f"Unknown key: {key}. Valid: gateway, mnemonic, evm, solana"))
        return
    ...
    if key == "gateway":
        if not value.startswith(("http://", "https://")):
            click.echo(_Colors.error("Gateway URL must start with http:// or https://"))
            return
            ...
            if is_secret:
                if has_keyring:
                    ...
                    if stored:
                        ...
                        click.echo(_Colors.success(f"✅ {key} stored securely in OS keyring"))
                    else:
                    ...
                    click.echo(click.style("    Install with: pip install keyring", fg="yellow"))
                    _set_config(key, value)
            else:
                _set_config(key, value)
                click.echo(_Colors.success(f"✅ Configured: {key}"))
...
def config_get(key):
    ...
    if key:
        ...
        click.echo(_Colors.info("📋 Current configuration:"))
        click.echo("")
...
def config_clear(ctx, assume_yes):
    ...
    if assume_yes_val or click.confirm(_Colors.warning("Clear all config? This cannot be undone.")):
        # Clear keyring
        ...
        # Clear file config
        _save_config({})
        click.echo(_Colors.success("✅ Config cleared"))
...
def completion(shell):
    ...
    This prints the completion script to stdout. Source it in your shell config:
      eval \"$(ipfs-pin completion bash)\"      # bash
      eval \"$(ipfs-pin completion zsh)\"        # zsh
      ipfs-pin completion fish | source         # fish
    """
    if click is None:
        click.echo("Error: click package not installed")
        return
    try:
        ...
        comp = shell_class(
            cli=cli,
            ctx_args={"color": False},
            prog_name="ipfs-pin",
            complete_var="_IPFS_PIN_COMPLETE",
        )
        # Check if this is a __complete__ call (sys.argv contains __complete__)
        if "__complete__" in sys.argv:
            # Respond with completions for the asked args
            args = sys.argv[sys.argv.index("__complete__") + 1:]
            result = comp.get_completion(args=args)
            if result:
                click.echo(result.result)
        else:
            # Generate and print the shell script
            click.echo(comp.source())
    except Exception as e:
        click.echo(_Colors.error(f"Failed to generate completion: {e}"))
        import traceback
