...
def test_prod_hono_e2e():
    ...
    if "extensions" in x402_spec:
        print("\n--- DEBUG: Extensions received in 402 Challenge ---")
    ...
    print("Response Status Code:", pinned_resp.status_code)
    try:
        print("Response Data:", json.dumps(pinned_resp.json(), indent=2))
    except:
        print("Response Data:", pinned_resp.text)
        
    ext_resp = pinned_resp.headers.get("EXTENSION-RESPONSES")
    if ext_resp:
        print("\n--- DEBUG: EXTENSION-RESPONSES header from Facilitator ---")
        print(json.dumps(json.loads(base64.b64decode(ext_resp).decode('utf-8')), indent=2))
        print("----------------------------------------------------------\n")
    else:
        print("\n--- DEBUG: No EXTENSION-RESPONSES header returned ---\n")
