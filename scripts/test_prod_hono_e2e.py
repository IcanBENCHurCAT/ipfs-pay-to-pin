...
def test_prod_hono_e2e():
    ...
    print(f"Created 1KB Green Square Image: {len(img_bytes)} bytes")

    resp = requests.post(
        f"{PROD_URL}/api/v1/pin",
        files={"file": ("green_square_prod_hono.png", img_bytes, "image/png")}
    )
    ...
    print("\n2. Submitting file + PAYMENT-SIGNATURE header to Production Gateway...")
