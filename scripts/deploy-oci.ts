...
let sshPublicKey = env.OCI_SSH_PUBLIC_KEY;
if (!sshPublicKey || sshPublicKey.includes('...')) {
  const homeDir = process.env.USERPROFILE || process.env.HOME || 'C:/Users/Garret';
  const edPub = path.join(homeDir, '.ssh', 'id_ed25519.pub');
  const rsaPub = path.join(homeDir, '.ssh', 'id_rsa.pub');
  if (fs.existsSync(edPub)) {
    sshPublicKey = fs.readFileSync(edPub, 'utf-8').trim();
  } else if (fs.existsSync(rsaPub)) {
    sshPublicKey = fs.readFileSync(rsaPub, 'utf-8').trim();
  } else {
    sshPublicKey = 'ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIL6VcbJ2GC5eWXns8QwKvef0LGuL7QpR7P2vGWs4djO7 garret@NucBox_k11';
  }
}
