...
def test_hono_e2e():
    print(f"1. Requesting Payment Challenge from local @x402/hono server ({LOCAL_URL})...")
    ...
    resp = requests.post(
        LOCAL_URL,
        files={"file": ("hono_cyan_badge.png", img_bytes, "image/png")}
    )
    ...
    print("\n" + "="*60)
    print("OFFICIAL @x402/hono CHALLENGE RECEIVED:")
    print("="*60)
    ...
    print("\n2. Submitting file + PAYMENT-SIGNATURE header to @x402/hono server...")
    ...
    if pinned_resp.status_code in (200, 201):
        print("\n" + "="*60)
        print("@x402/hono END-TO-END VERIFICATION & PINNING SUCCESSFUL!")
        print("="*60)
