...
def main():
    ...
    print(f"\nSending OpenClaw avatar to {PROD_URL}/api/v1/pin...")
    resp = requests.post(
        f"{PROD_URL}/api/v1/pin",
        files={"file": ("openclaw_lobster_avatar.jpg", img_bytes, "image/jpeg")}
    )
    ...
    print("\n" + "="*60)
    print("OPENCLAW AVATAR 402 PAYMENT CHALLENGE RECEIVED:")
    print("="*60)
    ...
    if verify_resp.status_code in (200, 201):
        print("\n" + "="*60)
        print("OPENCLAW LOBSTER AVATAR PINNED TO IPFS SUCCESSFULLY!")
        print("="*60)
